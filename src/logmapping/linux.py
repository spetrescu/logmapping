...
from logmapping.utils import LINUX_MAPPING_LEVELS, LINUX_MAPPING_TEMPLATE_GENERIC_VAR, LINUX_MAPPING_TEMPLATE_ENTITY, LINUX_MAPPING_TEMPLATE_UNDERLYING_STATEMENT, LINUX_VARIBLE_TUPLES
import csv

class LogMapperLinux:
    ...
    def remove_meta_information(self):
        ...
        for runtime_log in self.runtime_logs:
            if "combo" in runtime_log:
                runtime_log = "combo".join(runtime_log.split("combo")[1:]).lstrip().rstrip()
    ...
    def log_mapping_linux(self, level):
        ...
        for el in self.runtime_logs_no_meta:
            match = True
            for constant_part in LINUX_MAPPING_LEVELS[f"LEVEL_{level}"]:
        ...
        gdth_simple = LINUX_MAPPING_TEMPLATE_GENERIC_VAR[f"LEVEL_{level}"]
        gdth_rich = LINUX_MAPPING_TEMPLATE_ENTITY[f"LEVEL_{level}"]

        underlying_logging_statement = LINUX_MAPPING_TEMPLATE_UNDERLYING_STATEMENT[f"LEVEL_{level}"]

        for runtime_log in matches_runtime:
            mapped_runtime_logs_to_loggging_statements.append([runtime_log, underlying_logging_statement, gdth_rich, gdth_simple, LINUX_VARIBLE_TUPLES[f"LEVEL_{level}"]])
    ...
    def write_mapping_to_file(self, level, type):
        ...
        spamWriter = csv.writer(
            open(f"{type}_dataset/{type}_dataset_linux_level_{level}.csv", "w")
        )
        for r, g, e, v in zip(
            runtime_log,
            ground_truth,
