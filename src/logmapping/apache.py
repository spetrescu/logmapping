...
from logmapping.utils import APACHE_MAPPING_LEVELS, APACHE_MAPPING_TEMPLATE_GENERIC_VAR, APACHE_MAPPING_TEMPLATE_ENTITY, APACHE_MAPPING_TEMPLATE_UNDERLYING_STATEMENT, APACHE_VARIBLE_TUPLES
import csv

class LogMapperApache:
    ...
    def log_mapping_apache(self, level):
        ...
        for el in self.runtime_logs_no_meta:
            match = True
            for constant_part in APACHE_MAPPING_LEVELS[f"LEVEL_{level}"]:
        ...
        gdth_simple = APACHE_MAPPING_TEMPLATE_GENERIC_VAR[f"LEVEL_{level}"]
        gdth_rich = APACHE_MAPPING_TEMPLATE_ENTITY[f"LEVEL_{level}"]

        underlying_logging_statement = APACHE_MAPPING_TEMPLATE_UNDERLYING_STATEMENT[f"LEVEL_{level}"]

        for runtime_log in matches_runtime:
            mapped_runtime_logs_to_loggging_statements.append([runtime_log, underlying_logging_statement, gdth_rich, gdth_simple, APACHE_VARIBLE_TUPLES[f"LEVEL_{level}"]])
    ...
    def write_mapping_to_file(self, level, type):
        ...
        spamWriter = csv.writer(
            open(f"{type}_dataset/{type}_dataset_apache_level_{level}.csv", "w")
        )
        for r, g, e, v in zip(
            runtime_log,
            ground_truth,
