...
JAVA_LOGGING_FORMULAE_logger = ["log.trace(",
                                "log.debug(",
                                "log.info(",
                                "log.warn(",
                                "log.error(",
...
def generate_log_templates_with_generic_tokens_from_raw_logging_statements(logging_statements):
    print(f"Now processing {len(logging_statements)} logging statements to generate log templates with generic tokens only...")
    ...
    df.to_csv(save_path)
    print(f"Generated {len(processed_lines)} generic log templates... Can be found under {save_path}\n")

    return generic_templates


def generate_log_templates_with_rich_tokens_from_raw_logging_statements(logging_statements):
    print(f"Now processing {len(logging_statements)} logging statements to generate log templates with generic tokens only...")
    ...
    df.to_csv(save_path)
    print(f"Generated {len(processed_lines)} rich log templates... Can be found under {save_path}\n")
...
def find_logging_statements(files_with_logging_statements, language):
                ...
                if formula in line:
                    ...
                    if not line.endswith(LINE_TERMINATOR_SPECIFIC_LANGUAGE[language]):
                        ...
                        while True:
                            ...
                            if not warning_line.endswith(LINE_TERMINATOR_SPECIFIC_LANGUAGE[language]):
                        ...
                        logging_statements_found.append([line.strip(), file.split("/")[-1]])
    ...
    print("Multiple-line logging statements: ", len(logging_statements_on_multiple_lines))
...
def find_code_files(path, file_extension):
    ...
    print(f"Found {len(list_of_all_files)} code files with the {file_extension} extension\n")
...
def find_files_that_contain_logging_statements(list_of_code_files, logging_formulae):
    ...
    print(f"Found {len(list(set(files_with_logging_statements)))} files that contain logging statements\n")
...
def match_runtime_log_to_template(runtime_logs, templates):
    ...
    logs_matched = list(zip(raw_templates_matched, processed_templates_matched, runtime_logs_matched))
    df = pd.DataFrame(logs_matched, columns=[['raw_templates_matched', 'processed_templates_matched', 'runtime_logs_matched']])
    df = df.drop_duplicates()
    save_path = "refactored_code_output/mapped_logs.csv"
    df.to_csv(save_path, index=False)
    print(f"Matched {len(logs_matched)} runtime logs to logging statements... Can be found under {save_path}\n")
    ...
    for raw, proc_temp, runt_log in zip(raw_templates_matched, processed_templates_matched, runtime_logs_matched):
        ...
        if len(constants_placed) > 1 or "<GENERIC_VAR>" in constants_in_template:
            for i in range(len(constants_placed)):
                ...
                if i == (len(constants_placed) - 1) and constants_placed[i][2] < len(save_runt):
...
def map_runtime_logs_logging_statements(path_to_dir_containing_codebase, path_to_log_dataset, language):
    list_of_code_files = find_code_files(path=path_to_dir_containing_codebase, file_extension=CODE_EXTENSIONS_SPECIFIC_LANGUAGE[language])

    list_of_files_that_contain_logging_statements = find_files_that_contain_logging_statements(list_of_code_files=list_of_code_files, logging_formulae=FORMULAE_LOGGING_STATEMENTS_SPECIFIC_LANGUAGE[language])
    
    logging_statements = find_logging_statements(files_with_logging_statements=list_of_files_that_contain_logging_statements, language=language)

    generic_templates = generate_log_templates_with_generic_tokens_from_raw_logging_statements(logging_statements)
    rich_templates = generate_log_templates_with_rich_tokens_from_raw_logging_statements(logging_statements)
    ...
    raw_templates_matched, processed_templates_matched, runtime_logs_matched = match_runtime_log_to_template(runtime_logs, generic_templates)

    create_training_data_for_ner_model(raw_templates_matched, processed_templates_matched, runtime_logs_matched)
