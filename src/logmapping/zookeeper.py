...
from logmapping.utils import ZOOKEEPER_MAPPING_LEVELS, ZOOKEEPER_MAPPING_TEMPLATE_GENERIC_VAR, ZOOKEEPER_MAPPING_TEMPLATE_ENTITY, ZOOKEEPER_MAPPING_TEMPLATE_UNDERLYING_STATEMENT, ZOOKEEPER_VARIBLE_TUPLES
import csv

class LogMapperZookeeper:
    ...
    def log_mapping_zookeeper(self, level):
        ...
        for el in self.runtime_logs_no_meta:
            match = True
            for constant_part in ZOOKEEPER_MAPPING_LEVELS[f"LEVEL_{level}"]:
        ...
        gdth_simple = ZOOKEEPER_MAPPING_TEMPLATE_GENERIC_VAR[f"LEVEL_{level}"]
        gdth_rich = ZOOKEEPER_MAPPING_TEMPLATE_ENTITY[f"LEVEL_{level}"]

        underlying_logging_statement = ZOOKEEPER_MAPPING_TEMPLATE_UNDERLYING_STATEMENT[f"LEVEL_{level}"]

        for runtime_log in matches_runtime:
            mapped_runtime_logs_to_loggging_statements.append([runtime_log, underlying_logging_statement, gdth_rich, gdth_simple, ZOOKEEPER_VARIBLE_TUPLES[f"LEVEL_{level}"]])
    ...
    def write_mapping_to_file(self, level, type):
        ...
        spamWriter = csv.writer(
            open(f"{type}_dataset/{type}_dataset_zookeeper_level_{level}.csv", "w")
        )
        for r, g, e, v in zip(
            runtime_log,
            ground_truth,
